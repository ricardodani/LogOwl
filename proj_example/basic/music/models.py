...
from logowl.models import LogOwlModel

class Genre(LogOwlModel):
    """Genre model"""
...
class Band(LogOwlModel):
    """Band model"""
    title = models.CharField(max_length=100)
    prefix = models.CharField(max_length=20, blank=True)
    slug = models.SlugField(unique=True)
    musicians = models.ManyToManyField(Person, blank=True,
        limit_choices_to={'person_types__slug__exact': 'musician'})
    website = models.URLField(blank=True, verify_exists=False)
